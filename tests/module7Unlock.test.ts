...
describe("Contenu et déverrouillage du Module 7", () => {
  const course = getCourse("claude-code");
  const requirements = getAllModuleRequirements("claude-code", course);
  const module7 = course.modules.find((item) => item.slug === "module-7")!;
  ...
  it("déclare 6 leçons et le Module 6 comme prérequis", () => {
    expect(module7.lessons).toEqual([
      "lesson-7-1",
      "lesson-7-2",
      "lesson-7-3",
      "lesson-7-4",
    ...
    expect(module7.prerequisites).toEqual(["module-6"]);
  });

  it("charge des JSON cohérents avec des quiz de 5 questions", () => {
    const practicalIds = new Set<string>();

    module7.lessons.forEach((lessonId) => {
      const lesson = getLesson("claude-code", "module-7", lessonId);
  ...
  it("reste verrouillé avant la validation complète du Module 6", () => {
    const snapshot = emptyProgress();
    ["module-1", "module-2", "module-3", "module-4", "module-5"].forEach(
    ...
    expect(progress.moduleProgress["module-7"].status).toBe("locked");
  });

  it("se débloque après la validation complète des Modules 1 à 6", () => {
    const snapshot = emptyProgress();
    ...
    const progress = computeCourseProgress(course, requirements, snapshot);
    expect(progress.moduleProgress["module-6"].status).toBe("completed");
