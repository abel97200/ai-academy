...
describe("Contenu et déverrouillage du Module 8", () => {
  const course = getCourse("claude-code");
  const requirements = getAllModuleRequirements("claude-code", course);
  const module8 = course.modules.find((item) => item.slug === "module-8")!;
  ...
  it("déclare 6 leçons et le Module 7 comme prérequis", () => {
    expect(module8.lessons).toEqual([
      "lesson-8-1",
      "lesson-8-2",
      "lesson-8-3",
      "lesson-8-4",
    ...
    expect(module8.prerequisites).toEqual(["module-7"]);
  });

  it("charge des JSON cohérents avec des quiz de 5 questions", () => {
    const practicalIds = new Set<string>();

    module8.lessons.forEach((lessonId) => {
      const lesson = getLesson("claude-code", "module-8", lessonId);
  ...
  it("reste verrouillé si le Module 7 est incomplet", () => {
    const snapshot = emptyProgress();
    ["module-1", "module-2", "module-3", "module-4", "module-5", "module-6"].forEach(
      (slug) => completeModule(snapshot, slug)
    );
    ["lesson-7-1", "lesson-7-2", "lesson-7-3"].forEach((id) =>
      snapshot.completedLessonIds.add(id)
    );

    const progress = computeCourseProgress(course, requirements, snapshot);
    expect(progress.moduleProgress["module-7"].status).toBe("current");
    expect(progress.moduleProgress["module-8"].status).toBe("locked");
  });

  it("se débloque après la validation complète du Module 7", () => {
    const snapshot = emptyProgress();
    [
      "module-1",
      "module-2",
      ...
      "module-6",
      "module-7",
    ].forEach((slug) => completeModule(snapshot, slug));

    const progress = computeCourseProgress(course, requirements, snapshot);
    expect(progress.moduleProgress["module-7"].status).toBe("completed");
